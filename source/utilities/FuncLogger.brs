    ...
    return {
        ...
        p: function (text = "" as string, _0 = "", _1 = "", _2 = "", _3 = "")
            ...
            if not m.trackerNode.muted then 
                _0 = convertToStr(_0, m.tab.len())
                _1 = convertToStr(_1, m.tab.len())
                _2 = convertToStr(_2, m.tab.len())
                _3 = convertToStr(_3, m.tab.len())
                text = substitute(text, _0, _1, _2, _3)
...
function convertToStr(obj, _tab = 0) 
    ...
    if type(obj) = "roArray" then 
        values = []
        for each val in obj
            values.push(appendTab(convertToStr(val,_tab + tabSize),_tab + tabSize))
        end for
    ...
    if type(obj) = "roAssociativeArray" then
        ...
        for each field in obj.keys()
            value = convertToStr(obj[field], _tab + tabSize)
